...
func Test_app_getTokenFromHeaderAndVerify(t *testing.T) {
	...
	var tests = []struct {
		...
		{"no bearer", fmt.Sprintf("Bear %sasdf1", tokens.Token), true, true, app.Domain},
		{"three header parts", fmt.Sprintf("Bearer %s 31", tokens.Token), true, true, app.Domain},
		// make sure the next test is the last one to run because the diff domain
