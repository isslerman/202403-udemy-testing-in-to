// put a tag to run only this test separated
// you need to run: go test -v -tags=integration
// space = comment tag //nospace = build tag
...
var (
	...
	password = "postgres"
	dbName   = "users_test"
	port     = "5435" // dif from default to not interfere with other tests
...
func TestMain(m *testing.M) {
	...
	if err := pool.Retry(func() error {
		...
		if err2 := pool.Purge(resource); err2 != nil {
			log.Fatalf("could not purge resource: %s", err2)
		}
...
func createTables() error {
	// read the file and store as a []bytes
